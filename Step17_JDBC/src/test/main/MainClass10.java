...
public class MainClass10 {
	...
	// 매개 변수에 전달된 MemberDTO 객체에 담긴 name 과 sddr 를 DB 에 저장하고
	// 작업의 성공 여부를 리턴하는 메소드
	public static boolean insert(MemberDTO dto) {
		...
		// 변화된 row 개수를 저장할 변수를 0을 대입해서 미리 만들어 둔다.
		int rowCount = 0;
		...
		try {
			...
			// sql 문 실행하고 추가(insert)되거나 변경(update)되거나 삭제(delete)된 row 의 개수 리턴 받기
		...
		// rowCount 변수에 들어 있는 값을 확인해서 작업의 성공 여부를 리턴해준다.
		if(rowCount>0) {
			return true;
		}else {
			return false;
